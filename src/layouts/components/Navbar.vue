  ...
  <div class="navbar-container d-flex content align-items-center">
    ...
    <b-navbar-nav class="nav align-items-center ml-auto">
      ...
      <b-dropdown class="ml-1" variant="link" no-caret toggle-class="p-0" right>
        ...
        <b-dropdown-item :to="{ name: 'portfolio' }" class="d-none">
          <feather-icon icon="PieChartIcon" size="16" />
    ...
    block() {
      ...
      this.$http.getLatestBlock().then((block) => {
        ...
        if (timeIn(block.block.header.time, 1, "m")) {
          this.variant = "danger";
          this.tips = `Halted ${toDay(block.block.header.time, "from")}, Height: ${
            this.$store.state.chains.height
          } `;
        } else {
