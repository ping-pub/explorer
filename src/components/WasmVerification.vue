...
<template>
  <div class="bg-vector-bg px-4 pt-3 pb-4 rounded mb-4 shadow">
    <div role="tablist" class="tabs tabs-boxed">
      ...
      <a role="tab" class="tab" :class="{ 'tab-active': tab === 'verification' }"
        @click="selectTab('verification')">Verification</a>
      <a role="tab" class="tab" :class="{ 'tab-active': tab === 'executions' }"
        @click="selectTab('executions')">Functions</a>
      <a role="tab" class="tab" :class="{ 'tab-active': tab === 'source_code' }" @click="selectTab('source_code')">Source
        Code</a>
    </div>
    <div class="">
            ...
            <div v-for="(props, method) in m.properties"
              ...
              <div class="collapse-content">
                <div v-for="(p, name) in props.properties" class="form-control pb-2">
                  ...
                  <input :name="`${method}-${name}`" type="text" :placeholder="p.format"
                    class="input input-sm border border-gray-300 dark:border-gray-600 w-full" />
                </div>
                <div>
                  <label v-if="title === 'ExecuteMsg'" for="wasm_execute_contract" class="btn btn-sm"
                    @click="callFunction(title, method, props)">{{ method }}</label>
                  <label v-else class="btn btn-sm" @click="callFunction(title, method, props)">{{ method }}</label>
                </div>
                <div v-if="result[`${title}-${method}`]" class="mt-2">
                  <JsonViewer :value="result[`${title}-${method}`]" :theme="baseStore.theme || 'dark'"
      ...
      <div v-if="tab === 'source_code'" class="mt-2 join join-vertical w-full">
        <div v-for="sc in sourceCode.filter(x => !x.isDirectory)"
          class="collapse collapse-arrow join-item border border-base-300">
    ...
    <div v-show="tab === 'verification'" class="text-center">
      ...
      <button class="btn btn-primary mt-5" @click="verify" v-show="tab === 'verification'"
        :disabled="verification.error !== undefined">verify</button>
    </div>
    ...
    <div class="text-[#00cfe8] bg-[rgba(0,207,232,0.12)] rounded shadow mt-4 alert-info">
      ...
      <div class="px-4 py-4">
        <ul style="list-style-type: disc" class="pl-8">
          ...
          <li>
            <a href="https://docs.welldonestudio.io/code/verification-api/" target="_blank">Link to Verification API
              Manual</a>
          </li>
        </ul>
      </div>
