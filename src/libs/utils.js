...
export async function connectLedger(transport = 'usb') {
  const trans = (await transport) === 'usb'
...
export async function sign(
  ...
  const addr = device.startsWith('ledger') && coinType === 118
    ? toSignAddress(signerAddress)
    : signerAddress
...
export function isTestnet() {
  return (
    ...
    || process.env.VUE_APP_FORCE_TESTNET_MODE
  )
}
