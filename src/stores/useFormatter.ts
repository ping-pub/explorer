...
import type { Coin, DenomTrace, DenomMetadata } from '@/types';
...
export const useFormatter = defineStore('formatter', {
  state: () => {
    return {
      ...
      loading: [] as string[],
      denomsMetadata: [] as DenomMetadata[],
    };
  },
  getters: {
    blockchain() {
  ...
  actions: {
    async initial() {
      this.denomsMetadata = (await this.blockchain.rpc.getBankDenomMetadata()).metadatas
    },
    ...
    async fetchDenomMetadata(denom: string) {
      if (!this.ibcMetadata[denom]) {
        const denomMetadata = this.denomsMetadata.find((x) =>
          x.base.endsWith(denom)
        );
        if (denomMetadata) {
          this.ibcMetadata[denom] = denomMetadata;
        }
        const asset = this.findGlobalAssetConfig(denom)
        if (asset) {
          const display = this.ibcMetadata[denom]?.display
          this.ibcMetadata[denom] = { ...this.ibcMetadata[denom], ...asset };
          this.ibcMetadata[denom].display = display
        }
      }
      return this.ibcMetadata[denom];
    },
