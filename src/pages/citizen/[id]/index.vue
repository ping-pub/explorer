...
onMounted(async () => {
  listCitizen.fetchCitizen(numberFromUrl);
});
const dataCitizen = computed(() => {
  return listCitizen.citizen;
});
</script>
<template>
  <div class="bg-base-100 px-4 pt-3 pb-4 rounded mt-5">
    ...
    <div class="grid grid-cols-2 md:!grid-cols-4 lg:!grid-cols-5 2xl:!grid-cols-6 gap-4">
      ...
      <div class="rounded-sm bg-active px-4 py-2">
        ...
        <div class="text-base text-main">{{dataCitizen.userName}}</div>
      </div>
      <div class="rounded-sm bg-active px-4 py-2">
        <div class="text-xs mb-2 text-secondary capitalize">Level</div>
        <div class="text-base text-main">{{dataCitizen.level}}</div>
      </div>
      <div class="rounded-sm bg-active px-4 py-2">
        <div class="text-xs mb-2 text-secondary capitalize">Address Wallet</div>
        <div class="text-base text-main">{{dataCitizen.addressWallet}}</div>
      </div>
       <div class="rounded-sm bg-active px-4 py-2">
        <div class="text-xs mb-2 text-secondary capitalize">NFT Core</div>
        <div class="text-base text-main">{{dataCitizen.nftName}}</div>
      </div>
       <div class="rounded-sm bg-active px-4 py-2">
        <div class="text-xs mb-2 text-secondary capitalize">Balance</div>
        <div class="text-base text-main">{{dataCitizen.balance}}</div>
      </div>
    </div>
  </div>
</template>
