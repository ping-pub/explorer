...
import { MsgDelegate } from "../../utils/ledger/getTx";
  ...
  data() {
    return {
      ...
      form: {
        amount: 0,
        toAddress: "",
        memo: "stake via look.ping.pub 2020"
      },
      address: "",
      account: {
        account_number: "",
  ...
  methods: {
    ...
    sendByLedger() {
      if (!this.form.toAddress || !this.form.amount) {
        this.noticeMsg = "Please input form.";
        return;
      }
      this.signLedger();
    },
    async signLedger() {
      ...
      this.step = "load";
      try {
        txBroadcast = await MsgDelegate(signedMsg);
      } catch (e) {
      ...
      this.txhash = txhash;
      this.noticOk = "Successful signature and Transaction submitted!";
      setTimeout(async () => {
        await this.checkTx();
      }, 5000);
    },
