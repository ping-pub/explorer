<template>
  <div class="border-eee" v-loading="loading">
    <div class="bg-white governance-item" v-for="(item, index) of list" :key="index">
      <div class="flex-1">
        <div class="mb-10">
          <el-link
            :href="
              `#/committee/${item.id + (address ? '?address=' + address : '')}`
            "
            :disabled="item.id === '--'"
          >
        ...
        <div class="mb-10">
          ...
          <el-tag class="mr-10" type="info" size="mini">members: {{item.members && item.members.length}}</el-tag>
          <el-tag class="mr-10" type="info" size="mini">permissions: {{item.permissions && item.permissions.length}}</el-tag>
        </div>
        <div class="governance-text">{{ item.description }}</div>
      ...
      <!-- <RateBar
        v-if="item.id !== '--' && item.proposal_status !== 'VotingPeriod'"
